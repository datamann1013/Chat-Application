...
public class ChatClientGUI2 extends JFrame {
    ...
    private void onMessageReceived(String message) {
        SwingUtilities.invokeLater(() -> {
            ...
            if (message.startsWith("User joined: ") && message.length() >= 13) {
                // Handle user join message
                String userName = message.substring(13);
                if (!onlineUsersTextArea.getText().contains(userName)) {
                    onlineUsersTextArea.append(userName + "\n");
                }
                client.userJoined(userName);
            } else if (message.startsWith("User left: ") && message.length() >= 11) {
            ...
            } else if (message.startsWith("Online users: ") && message.length() >= 13) {
